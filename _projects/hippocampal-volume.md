...
## Challenge
Alzheimer's progression often involves hippocampal atrophy, and MRI volumetry is one of the more reliable ways to track it. The problem is that manual slice-by-slice segmentation is slow, rater-dependent, and hard to scale for longitudinal monitoring.

The task was to build a reproducible pipeline that curates and validates imaging data, trains a 3D-aware segmentation model with strong overlap to reference masks, and demonstrates how automated outputs could flow into a PACS-like environment with structured reporting, without claiming regulatory clearance but showing a credible research-to-integration path.
...
2. **Modeling** - Recursive U-Net with skip connections; 64x64 2D patches; Adam, cross-entropy; 10 epochs; experiment logging for iteration.
...
- Deliverables: three stages, curated data and EDA, trained model and `results.json`, Section 3 path with sample report artifacts and deploy scripts for end-to-end testing.
